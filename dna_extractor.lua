...
local function is_compostable(input)
	if minetest.get_item_group(input, 'tissue') > 0 or minetest.get_item_group(input, 'tissue') > 0 then
		return true
	else
		return false
	end
...
local function update_nodebox(pos)
	if is_empty(pos) then
		swap_node(pos, "prehistoric_life:dna_extractor_empty")
	else
		swap_node(pos, "prehistoric_life:dna_extractor")
	end
end
...
local function update_timer(pos)
	...
	if timer:is_started() and count < 1 then
		timer:stop()
		meta:set_string('infotext', 'DNA Extractor')
		meta:set_int('progress', 0)
	end
end
...
local function create_compost(pos)
	...
	for k, v in pairs(stacks) do
        ...
        local options = { "prehistoric_life:dna_vile"}
...
local function on_rightclick(pos, node, clicker, itemstack)
	minetest.show_formspec(
		...
		'prehistoric_life:dna_extractor',
		formspec(pos)
	)
end
...
minetest.register_node("prehistoric_life:dna_extractor_empty", {
	description = "DNA Extractor",
	tiles = {
		"prehistoric_life_dna_extractor1.png",
		"prehistoric_life_dna_extractor3.png",
		"prehistoric_life_dna_extractor2.png",
		"prehistoric_life_dna_extractor2.png",
		"prehistoric_life_dna_extractor2.png",
		"prehistoric_life_dna_extractor2.png"
	},
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-0.4375, -0.5, -0.4375, 0.4375, 0.0625, 0.0625}, -- NodeBox1
			{-0.4375, -0.5, 0.375, 0.4375, 0.375, 0.4375}, -- NodeBox2
			{0.375, -0.5, 0, 0.4375, 0.0625, 0.4375}, -- NodeBox3
			{-0.4375, -0.5, 0, -0.375, 0.0625, 0.4375}, -- NodeBox4
			{-0.4375, -0.5, 0, 0.4375, -0.3125, 0.4375}, -- NodeBox5
		}
	}
})

minetest.register_node("prehistoric_life:dna_extractor", {
	description = "DNA Extractor",
	tiles = {
		"prehistoric_life_dna_extractor1.png",
		"prehistoric_life_dna_extractor3.png",
	...
	node_box = {
		...
		fixed = {
			{-0.4375, -0.5, -0.4375, 0.4375, 0.0625, 0.0625}, -- NodeBox1
			{-0.4375, -0.5, -0.4375, -0.375, 0.0625, 0.4375}, -- NodeBox2
			{-0.4375, -0.5, 0.375, 0.4375, 0.0625, 0.4375}, -- NodeBox3
			{0.375, -0.5, -0.4375, 0.4375, 0.0625, 0.4375}, -- NodeBox4
			{-0.4375, -0.5, -0.4375, 0.4375, -0.4375, 0.4375}, -- NodeBox5
			{-0.4375, 0, 0.0625, 0.4375, 0.0625, 0.4375}, -- NodeBox6
		}
	}
})
...
minetest.register_craftitem("prehistoric_life:tissue", {
	...
	inventory_image = "prehistoric_life_soft_tissue.png",
	groups = {tissue=1}
	})

minetest.register_craft({
	output = "prehistoric_life:dna_extractor_empty",
	recipe = {
